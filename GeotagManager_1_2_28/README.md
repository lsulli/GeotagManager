...
**Version 1.2.8** | QGIS 3.x | Windows / macOS / Linux
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  TOOLBAR: Load Folder | Load GPX | Export GPKG ☐ Write EXIF | ...  │
├──────────────────────────┬──────────────────────┬───────────────────┤
│  LEFT PANEL              │  PHOTO PREVIEW        │  RIGHT PANEL      │
│                          │                       │                   │
│  ┌─ Sync from layer ─┐   │  [photo preview]      │  ┌─ Photo Info ─┐ │
│  │ [Connect] status  │   │                       │  │ filename     │ │
│  └───────────────────┘   │  ◀ Prev  1/52  Next ▶ │  │ source/date  │ │
│                          ├──────────────────────┤  │ altitude     │ │
│  Photos  📂 Open folder  │                       │  │ direction    │ │
│  [photo list]            │                       │  │ camera/FL    │ │
│  Total: 52 | Matched: 47 │                       │  └──────────────┘ │
│                          │                       │  ┌─ Map nav ────┐ │
│  GPX Tracks              │                       │  │ Scale widget │ │
│  [+ Add] [- Remove]      │                       │  │ 🔍 Zoom      │ │
│  [gpx file list]         │                       │  └──────────────┘ │
│  Total track points: 1842│                       │  ┌─ Edit coords─┐ │
│  ▶ Geotag photos from GPX│                       │  │ 📌 Move Point│ │
│                          │                       │  │ 🧭 Direction  │ │
│                          │                       │  │ Lat/Lon/Alt  │ │
│                          │                       │  │ ✔ Apply      │ │
│                          │                       │  │ ✏ Write EXIF │ │
│                          │                       │  └──────────────┘ │
├──────────────────────────┴──────────────────────┴───────────────────┤
│  Clock offset (sec) | Max gap (sec) | 👤 Authors | ☐ Symbology      │
├─────────────────────────────────────────────────────────────────────┤
│  Status message                          EXIF: ExifTool 13.54       │
└─────────────────────────────────────────────────────────────────────┘
```
